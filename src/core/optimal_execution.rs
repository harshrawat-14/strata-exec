...
impl AlmgrenChriss {
    ...
    fn sinh_schedule(&self, kappa: f64) -> Vec<f64> {
        ...
        let mut trades: Vec<f64> = (0..t)
            .map(|k| inventory[k] - inventory[k + 1])
            .collect();
...
mod tests {
    ...
    fn horizon_override_works() {
        let ac = AlmgrenChriss::new(1000.0, 0.1, 0.01, 0.02, Some(25))
            .expect("valid params");
    ...
    fn recommended_horizon_finite() {
        let ac = AlmgrenChriss::new(1000.0, 0.1, 0.01, 0.02, None)
            .expect("valid params");
        let h = ac.horizon();
    ...
    fn linear_schedule_when_lambda_zero() {
        let ac = AlmgrenChriss::new(1000.0, 0.1, 0.0, 0.02, Some(10))
            .expect("valid params");
    ...
    fn schedule_sum_equals_inventory() {
        ...
        let ac = AlmgrenChriss::new(5000.0, 0.05, 0.1, 0.03, Some(20))
            .expect("valid params");
        let sched = ac.schedule();
    ...
    fn schedule_sum_equals_inventory_auto_horizon() {
        let ac = AlmgrenChriss::new(10000.0, 0.1, 0.05, 0.02, None)
            .expect("valid params");
        let sched = ac.schedule();
        let sum: f64 = sched.iter().sum();
        assert!(
            (sum - 10000.0).abs() < 1e-9,
            "sum {sum} != 10000"
        );
    ...
    fn optimal_schedule_front_loads() {
        ...
        let ac = AlmgrenChriss::new(1000.0, 0.1, 0.5, 0.05, Some(10))
            .expect("valid params");
        let sched = ac.schedule();
        assert!(
            sched[0] > sched[9],
